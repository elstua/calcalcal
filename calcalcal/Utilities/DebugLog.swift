...
///
/// Behaves like Swift's global `dlog(...)` in Debug builds and compiles to a
/// no-op in Release builds, so console logging cannot leak into production.
///
/// Migration history: every `dlog(...)` call in the app source was renamed to
...
public func dlog(
    ...
    Swift.dlog(line, terminator: terminator)
    #endif
}
